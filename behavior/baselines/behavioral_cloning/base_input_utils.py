...
class BHDataset(object):
    def __init__(self, spec_file):
        ...
        self.actions, self.proprioceptions, self.rgbs, self.task_obss = read_proc_parallel(self.files)
...
def read_spec_file(fname):
    ...
    for line in f:
        items = line.split()
        if len(items) == 0 or line.startswith("#"): continue
        elif items[0] == "DIR": dir_proc = items[1]
        else:
...
def read_proc_parallel(files):
    ...
    actions, proprioceptions, rgbs, task_obss = np.empty((0,ACT_DIM)), np.empty((0,PROPRIOCEPTION_DIM)),np.empty((0,IMG_DIM,IMG_DIM,3)), np.empty((0,TASK_OBS_DIM))
    for f in files:
