...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script to batch-replay segmented demos using motion primitives."
    )
    parser.add_argument(
        "demo_directory", type=str, help="Path to directory containing demos"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "results_directory", type=str, help="Path to directory to store results in"
    )
...
def main():
    ...
    for demo_fullpath in tqdm.tqdm(
        list(glob.glob(os.path.join(args.segmentation_directory, "*.json")))
    ):
